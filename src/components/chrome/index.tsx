...
import LogoSvg from './assets/logo_dm.svg';
...
export default function Chrome(props: IChromeProps) {
  ...
  const [openGroups, setOpenGroups] = useState(
    JSON.parse(String(localStorage.getItem('openNavGroups'))) || ['DataMax'],
  );
    ...
    >
      ...
      <EuiFlexItem className="eui-yScroll">
        ...
        <EuiCollapsibleNavGroup
          ...
          isCollapsible={true}
          initialIsOpen={openGroups.includes('Ext')}
          onToggle={(isOpen: boolean) => toggleAccordion(isOpen, 'Ext')}
        >
          <EuiPinnableListGroup
            aria-label="Ext" // A11y : EuiCollapsibleNavGroup can't correctly pass the `title` as the `aria-label` to the right HTML element, so it must be added manually
